...
public static class TCP2_ShaderGeneratorUtils
{
	...
	static private string GenerateShaderSource(this TCP2_Config config, TCP2_ShaderGenerator.ShaderGeneratorTemplate template, Shader existingShader = null)
	{
		...
		foreach (KeyValuePair<string, string> kvp in config.Keywords)
		{
			...
			else if (kvp.Value == "IN.color" || kvp.Value == "vcolor")
			{
		...
		foreach (KeyValuePair<string, string> kvp in config.Keywords)
		{
			if (kvp.Key == "UV_mask1") config.ToggleFeature("UVMASK1", kvp.Value == "Independent UV" && mask1);
			else if (kvp.Key == "UV_mask2") config.ToggleFeature("UVMASK2", kvp.Value == "Independent UV" && mask2);
			else if (kvp.Key == "UV_mask3") config.ToggleFeature("UVMASK3", kvp.Value == "Independent UV" && mask3);
		}
			...
			if(line.StartsWith("#"))
			{
				...
				if (line.StartsWith("#KEYWORDS"))
				{
					while (i < templateLines.Length)
					{
						...
						string error = ProcessKeywords(templateLines[i], ref features, ref flags, ref keywords, ref i, ref depth, ref stack, ref done);
	...
	static private string ProcessKeywords(string line, ref List<string> features, ref List<string> flags, ref Dictionary<string,string> keywords, ref int i, ref int depth, ref List<bool> stack, ref List<bool> done)
	{
		if (line.Contains("///"))
		{
