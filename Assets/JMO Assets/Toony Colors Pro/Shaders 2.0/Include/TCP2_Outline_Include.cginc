...
// (c) 2014-2017 Jean Moreno
	...
	//Handle new function from Unity 5.6, and avoid warnings
	float4 ObjectToViewPos(float4 vertex)
	{
#if UNITY_VERSION >= 560
		return float4(UnityObjectToViewPos(vertex), 1.0);
#else
		return float4(mul(UNITY_MATRIX_V, mul(unity_ObjectToWorld, vertex)).xyz, 1.0);
#endif
	}
...
//Correct Z artefacts
#if TCP2_ZSMOOTH_ON
	float4 pos = ObjectToViewPos(v.vertex);
	...
	#ifdef TCP2_OUTLINE_CONST_SIZE
		float dist = distance(_WorldSpaceCameraPos, mul(unity_ObjectToWorld, v.vertex));
		float4 pos = ObjectToViewPos(v.vertex + float4(normal, 0) * _Outline * 0.01 * dist);
	#else
		float4 pos = ObjectToViewPos(v.vertex + float4(normal, 0) * _Outline * 0.01);
	#endif
#endif
